...
1. **Модульность** - каждая сущность имеет свой набор классов
2. **Инкапсуляция** - работа с БД скрыта в моделях
3. **Автоматическое обновление** - изменения сразу отображаются в интерфейсе
4. **Транзакционность** - важные операции (продажа билетов) выполняются в транзакциях
5. **Валидация** - данные проверяются на нескольких уровнях

Программа обеспечивает полный CRUD (Create, Read, Update, Delete) функционал для всех сущностей железнодорожной системы с поддержкой связей между таблицами.
